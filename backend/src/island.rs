...
async fn unlock_specific(state: web::Data<Arc<AppState>>, user: AuthenticatedUser, island_code: web::Path<String>) -> AppResult<HttpResponse> {
    ...
    let allowed = ["abyss_island","golden_kingdom","void_star","eclipse","storm","time_labyrinth","eternity"];
    ...
    let (need_stage, need_gold, need_vip, need_awak, reward_skin) = match code.as_str() {
        "abyss_island" => (500,5000,0,0,"wings_t8_island_abyss"),
        "golden_kingdom" => (550,8000,5,0,"wings_t8_golden_kingdom"),
        "void_star" => (600,12000,8,0,"wings_t8_void_star"),
        "eclipse" => (650,15000,10,1,"wings_t8_eclipse"),
        "storm" => (700,18000,12,2,"wings_t8_storm"),
        "time_labyrinth" => (750,22000,13,3,"wings_t8_time"),
        "eternity" => (800,26000,14,4,"wings_t8_eternity"),
        _ => (500,5000,0,0,"wings_t8_island_abyss"),
    };
    ...
    let start_stage = match code.as_str() {
        "abyss_island" => 501, "golden_kingdom" => 551, "void_star" => 601, "eclipse" => 651, "storm" => 701, "time_labyrinth" => 751, "eternity" => 801, _ => 501,
    };
...
async fn enter(state: web::Data<Arc<AppState>>, user: AuthenticatedUser, stage: web::Path<u16>) -> AppResult<HttpResponse> {
    ...
    if !(501..=850).contains(&s) { return Err(AppError::Validation("Ilhas 501-850 apenas".into())); }
    let code = if s<=550 {"abyss_island"} else if s<=600 {"golden_kingdom"} else if s<=650 {"void_star"} else if s<=700 {"eclipse"} else if s<=750 {"storm"} else if s<=800 {"time_labyrinth"} else {"eternity"};

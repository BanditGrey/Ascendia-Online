...
async fn unlock_specific(state: web::Data<Arc<AppState>>, user: AuthenticatedUser, island_code: web::Path<String>) -> AppResult<HttpResponse> {
    ...
    if code != "abyss_island" && code != "golden_kingdom" && code != "void_star" && code != "eclipse" && code != "storm" { return Err(AppError::Validation("ilha deve ser abyss_island, golden_kingdom, void_star, eclipse ou storm".into())); }
    ...
    let (need_stage, need_gold, need_vip, need_awak, reward_skin) = if code=="abyss_island" { (500,5000,0,0,"wings_t8_island_abyss") } else if code=="golden_kingdom" { (550,8000,5,0,"wings_t8_golden_kingdom") } else if code=="void_star" { (600,12000,8,0,"wings_t8_void_star") } else if code=="eclipse" { (650,15000,10,1,"wings_t8_eclipse") } else { (700,18000,12,2,"wings_t8_storm") };
    ...
    let start_stage = if code=="abyss_island" {501} else if code=="golden_kingdom" {551} else if code=="void_star" {601} else if code=="eclipse" {651} else {701};
...
async fn enter(state: web::Data<Arc<AppState>>, user: AuthenticatedUser, stage: web::Path<u16>) -> AppResult<HttpResponse> {
    ...
    if !(501..=750).contains(&s) { return Err(AppError::Validation("Ilhas 501-750 apenas".into())); }
    let code = if s<=550 {"abyss_island"} else if s<=600 {"golden_kingdom"} else if s<=650 {"void_star"} else if s<=700 {"eclipse"} else {"storm"};
    ...
    let (enemies, boss) = if s<=550 { (vec!["abyssal_horror","deep_one"], s==550) } else if s<=600 { (vec!["golden_golem","treasure_mimic"], s==600) } else if s<=650 { (vec!["void_horror","star_wraith"], s==650) } else if s<=700 { (vec!["eclipse_horror","eclipse_wraith"], s==700) } else { (vec!["storm_horror","thunder_wraith"], s==750) };
    let res: serde_json::Value = serde_json::json!({"stage":s,"island":code,"enemies":enemies,"boss":boss,"note":"Use POST /api/v1/combat/start com stage 501-750"});
